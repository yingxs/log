...
* 场景1：每个线程都需要一个独享的对象，通常是非线程安全的工具类，比如；SimpleDateFormat、Random；这就需要ThreadLocal帮我们为每一个线程都制造一个独享的对象，不同的线程之间拥有的是不同的工具类实例对象，因此相互之间并不会影响
* 场景2：每个线程内需要保存全局变量，可以让不同方法直接使用，避免参数传递的麻烦

##### 3.1.1 场景1：每个线程需要一个独享的对象

* 每个Thread内有自己的实例副本，不共享
* 比喻：教材只有一本，一起做笔记有线程安全的问题，但是复印之后就没有问题了
